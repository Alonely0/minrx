...
fn test_bytes_pattern_and_haystack() {
    let re = Regex::new(b"a+b".as_slice()).unwrap();
...
fn test_case_insensitive_disabled_by_default() {
    let re = RegexBuilder::new().build("hello").unwrap();
    assert!(!re.is_match("HELLO").unwrap());
}
...
fn test_brace_compat_literal_brace() {
    let re = RegexBuilder::new().brace_compat(true).build("a{b").unwrap();
...
fn test_extended_no_op() {
    let builder = RegexBuilder::new().extended(false);
...
fn test_builder_default_matches_new() {
    let re = RegexBuilder::default().build("abc").unwrap();
    assert!(re.is_match("abc").unwrap());
}

#[test]
fn test_builder_reusable_across_multiple_builds() {
    let builder = RegexBuilder::new().case_insensitive(true);
    let re1 = builder.build("abc").unwrap();
    let re2 = builder.build("xyz").unwrap();
